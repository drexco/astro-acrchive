	...
	public static function editCurrency($currency_id)
	{
		if (Session::get('user_id')) {
			if (Session::get('account_type') == 'admin' ) {
				...
				if ($data) {
					$validator = AdminModel::validateCurrency();
					if($validator->fails()) {
						Session::flash('currency_message','Currency could not be added!');
						return Redirect::to('/admin/currencies')->withErrors($validator)->withInput();
					} else {
						Session::flash('currency_message','Currency Updated!');
			...
			if (Session::get('account_type') == 'admin' ) {
				...
				if ($data) {
					...
					if($validator->fails()) {
						...
						Session::flash('currency_message','Currency Added!');
						return Redirect::to('/admin/currencies');
					}
				} 
			}
